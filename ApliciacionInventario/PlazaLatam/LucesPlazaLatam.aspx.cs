...
using ApliciacionInventario.ProxyPlazaLat;
using ApliciacionInventario.ProxyModelo;
using ApliciacionInventario.Usuario;
using OfficeOpenXml;
using System.IO;

namespace ApliciacionInventario.PlazaLatam
{
    public partial class LucesPlazaLatam : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Page.IsPostBack == false)
                {
                    ...
                    Session["extension2"] = "";
                    Session["imageTemp2"] = "";
                    dtv = new DataView(ConvertToDataTable(objPlazaLat.ListarPlazaLat2()));
                    Session["Vista"] = dtv;
                    CargarDatos("");
        ...
        protected void btnGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var copyToPath = Server.MapPath(@"~\PlazaLatam\Fotos\" + (string)(Session["imageTemp"]));
                ...
                objPlazaLatBE.piso = Convert.ToInt16(txtPiso.Text);
                objPlazaLatBE.area = txtArea.Text;
                objPlazaLatBE.equipo = Convert.ToInt16(cboEquipo.SelectedValue.ToString());
                objPlazaLatBE.estado = cboEstado.SelectedValue.ToString();
                objPlazaLatBE.obs = txtObs.Text;
                objPlazaLatBE.activo = Convert.ToInt16(chkActivo.Checked);
                objPlazaLatBE.imagen = "~\\PlazaLatam\\Fotos\\" + (string)(Session["imageTemp"]);
                objPlazaLatBE.usu_registro = clsCredenciales.Usuario;

                if (objPlazaLat.InsertarPlazaLat(objPlazaLatBE) == true)
                {
                    if (copyToPath != "")
                    {
                        // Copy the file
        ...
        protected void btnGrabar2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var copyToPath = Server.MapPath(@"~\PlazaLatam\Fotos\" + (string)(Session["imageTemp2"]));
                ...
                if ((string)(Session["imageTemp2"]) != "")
                {
                    ...
                    objPlazaLatBE.imagen = "~\\PlazaLatam\\Fotos\\" + (string)(Session["imageTemp2"]);
                }
                else {
                    objPlazaLatBE = objPlazaLat.ConsultarPlazaLat(Convert.ToInt16(Convert.ToInt16(txtid.Text)));

                }

                objPlazaLatBE.id = Convert.ToInt16(txtid.Text);
                objPlazaLatBE.piso = Convert.ToInt16(txtPiso2.Text);
                objPlazaLatBE.area = txtArea2.Text;
                objPlazaLatBE.equipo = Convert.ToInt16(cboEquipo2.SelectedValue.ToString());
                objPlazaLatBE.estado = cboEstado2.SelectedValue.ToString();
                objPlazaLatBE.obs = txtObs2.Text;
                objPlazaLatBE.activo = Convert.ToInt16(chkActivo2.Checked);                
                objPlazaLatBE.usu_ult_modificacion = clsCredenciales.Usuario;

                if (objPlazaLat.ActualizarPlazaLat(objPlazaLatBE) == true)
                {
        ...
        protected void grvLuces_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Editar")
                {
                    ...
                    objPlazaLatBE = objPlazaLat.ConsultarPlazaLat(Convert.ToInt16(strCod));
                    txtid.Text = (objPlazaLatBE.id).ToString();
                    txtPiso2.Text = Convert.ToString(objPlazaLatBE.piso);
                    txtObs2.Text = objPlazaLatBE.obs;
                    txtArea2.Text = objPlazaLatBE.area;
                    ImageUpload2.ImageUrl = objPlazaLatBE.imagen;
                    chkActivo2.Checked = Convert.ToBoolean(objPlazaLatBE.activo);
                    ...
                    cboEquipo2.DataBind();
                    cboEquipo2.SelectedValue = (objPlazaLatBE.equipo).ToString();

                    cboEstado2.SelectedValue = objPlazaLatBE.estado;
        ...
        protected void btnDescargar_Click(object sender, EventArgs e)
        {
            try
            {
                String rutaarchivo = Server.MapPath("/") + @"\Plantillas\ListadoLucesPlazaLat.xlsx";
                DataTable dtProductos = new DataTable();
                dtProductos = ConvertToDataTable(objPlazaLat.ListarPlazaLat2());
                ...
                using (var pck = new OfficeOpenXml.ExcelPackage(new FileInfo(rutaarchivo)))
                {
                    ...
                    String filename = "ListaLucesPlazaLat." + DateTime.Today.ToShortDateString();
