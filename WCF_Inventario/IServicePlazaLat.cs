...
{
    // NOTA: puede usar el comando "Rename" del menú "Refactorizar" para cambiar el nombre de interfaz "IServicePlazaLat" en el código y en el archivo de configuración a la vez.
    [ServiceContract]
    public interface IServicePlazaLat
    {
        [OperationContract]
        PlazaLatBE ConsultarPlazaLat(Int16 id);

        [OperationContract]
        PlazaLatBE ConsultarPlazaLat2(Int16 id);

        [OperationContract]
        List<PlazaLatBE> ListarPlazaLat();

        [OperationContract]
        List<PlazaLatBE> ListarPlazaLat2();

        [OperationContract]
        Boolean InsertarPlazaLat(PlazaLatBE objPlazaLatBE);

        [OperationContract]
        Boolean ActualizarPlazaLat(PlazaLatBE objPlazaLatBE);

        [OperationContract]
        Boolean EliminarPlazaLat(Int16 id);
    }

    [DataContract]
    [Serializable]
    public class PlazaLatBE
    {
